...
des jeux d'entrainement (représentées par des points) et de test (représentées par des croix) selon les valeurs prises par les variables $X1$ et $X2$. \newline
Nous pouvons remarquer que les deux catégories ne sont pas totalement distinctes. Mais il semble que pour la catégorie $0$, $X1$ est généralement plus faible que pour la catégorie $1$ et à l'inverse, $X2$ est globalement plus élevée pour la catégorie $0$ que pour la $1$. Les histogrammes montrent que la variable $X2$ permet de mieux discriminer que $X1$ et que les deux catégories sont équitablement représentées. \newline
La figure permet également de noter que les jeux d'entrainement et de test sont similaires, ce qui est important pour obtenir un classifieur performant sur les données de test et pas uniquement sur celles d'entrainement.

```{r plot-4-data, fig.height = 6, fig.cap = "Visualisation des jeux d'entrainement et de test"}
...
gp <- ggplot(data = df) +
  ...
  labs(color = "Catégorie", shape = "Jeu", x = "X1", y = "X2") +
  theme(axis.title.x = element_blank(),
        ...
        plot.margin = margin(0, 0, 0, 0, "pt"))
gh1 <- ggplot(data = df, aes(x = V1, fill = cat)) +
  geom_histogram(alpha = 0.6, position = "identity") +
  labs(fill = "Catégorie", x = "X1", y = "Effectif") +
  theme(plot.margin = margin(0, 0, 0, 0, "pt"))
gh2 <- ggplot(data = df, aes(y = V2, fill = cat)) +
  geom_histogram(alpha = 0.6, position = "identity") +
  labs(fill = "Catégorie", y = "X2", x = "Effectif") +
  theme(plot.margin = margin(0, 0, 0, 0, "pt"))
